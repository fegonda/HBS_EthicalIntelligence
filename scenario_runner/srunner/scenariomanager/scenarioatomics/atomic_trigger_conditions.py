...
class InTriggerDistanceToVehicle(AtomicCondition):
    ...
    def update(self):
        ...
        #     print('distance:', distance, 'limit:', self._distance, 'new_status:',new_status, self.name)
...
class InVehicleState(AtomicCondition):
    def __init__(self,
                 actor,
                 targets,
                 view_angle=0,
...
class InTriggerDistanceToLocation(AtomicCondition):
    ...
    def update(self):
        ...
        if self._comparison_operator(distance, self._distance):
            new_status = py_trees.common.Status.SUCCESS
            # if self.name.startswith('ego'):
            #     import pdb; pdb.set_trace()

        # if self.name.startswith('egodrive'):
        #     print('distance:', distance, 'limit:', self._distance, 'new_status:',new_status, self._target_location, self.name)
        self.logger.debug("%s.update()[%s->%s]" % (self.__class__.__name__, self.status, new_status))
...
class InTriggerDistanceToLocationInFront(InTriggerDistanceToLocation):
    ...
    def update(self):
        ...
        # Wait for the vehicle to be in front
        actor_dir = actor_waypoint.transform.get_forward_vector()
        actor_other_dir = location - self._target_location
        dot_ve_wp = actor_dir.x * actor_other_dir.x + actor_dir.y * actor_other_dir.y + actor_dir.z * actor_other_dir.z

        in_front = False
        if dot_ve_wp > 0.0:
            in_front = True

        if in_front and in_trigger_location:
            new_status = py_trees.common.Status.SUCCESS
...
class WaitUntilClear(AtomicCondition):
    ...
    def __init__(self, actor, other_actor, clearance=10, name="WaitUntilClear"):
        """
        Init
        """
        super(WaitUntilClear, self).__init__(name)
        self._actor = actor
        self._other_actor = other_actor
        self._distance = clearance
        self._factor = max(EPSILON, 1)  # Must be > 0
    ...
    def update(self):
        ...
        if actor_location.distance(other_location) > self._distance:
            new_status = py_trees.common.Status.SUCCESS
